...
for j=offset+1:offset+StrSz(StrID)    
   ...
   if organid==4 then
     ...
     //Vo=[x1 y1 z1; x2 y2 z2;x3 y3 z3];
     //exchange x y 
     //tmp=Vo(:,1);Vo(:,1)=Vo(:,2);Vo(:,2)=tmp;
     //exchange x z , wrong
     //tmp=Vo(:,1);Vo(:,1)=Vo(:,3);Vo(:,3)=tmp;
     //exchange y z , wrong
     //tmp=Vo(:,2);Vo(:,2)=Vo(:,3);Vo(:,3)=tmp;
     //exchange v2 and v3
     //tmp=Vo(2,:);Vo(2,:)=Vo(3,:);Vo(3,:)=tmp;
     //exchange v2 and v1
     //tmp=Vo(2,:);Vo(2,:)=Vo(1,:);Vo(1,:)=tmp;
     //exchange v3 and v1
     //tmp=Vo(3,:);Vo(3,:)=Vo(1,:);Vo(1,:)=tmp;
     //tmp=Vo(:,2);Vo(:,2)=Vo(:,3);Vo(:,3)=tmp;
     //exchange v2 and v3
     //tmp=Vo(2,:);Vo(2,:)=Vo(3,:);Vo(3,:)=tmp;
     //exchange v2 and v1
     ...
     data2=[V data(j,21) data(j,22) 0];//write position and orietation, section area for internode etc. into the linear file, total 15 data, 'float'
